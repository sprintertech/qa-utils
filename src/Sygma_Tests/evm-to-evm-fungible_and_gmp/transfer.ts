...
async function saveTransferLog(transferResults: TransferResult[]): Promise<void> {
  ...
  const filename = path.join(reportDir, `transfer-log-fungible_and_gmp_${formattedDate}.json`);
