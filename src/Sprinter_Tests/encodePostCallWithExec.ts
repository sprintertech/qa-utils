...
async function callApi(sendTx:boolean ) {
    ...
    try {
      ...
      for (const approval of approvals) {
        ...
        const providerUrl = process.env[`PROVIDER_URL_${approvalChainID}`] || "";
      ...
      const providerUrl = process.env[`PROVIDER_URL_${transactionChainID}`] || "";
      ...
      if (mainData && mainTo) {
        ...
        console.error("Main transaction data is missing from the API response.");
      }
      }
    } catch (error) {
